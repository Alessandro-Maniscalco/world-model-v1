...
- The first excess-only retry shows that the motion-bonus shape matters, but it is not a winner yet. Resuming clean h16 step `1000` with `motion_loss_alpha=0.75`, `motion_loss_max_weight=2.0`, and excess-only weighting stayed plausible and improved episode-`1` fidelity over the cap-only runs, but the main clip and episode `2` still remained too late-heavy to beat the uncapped `0.5` h16 reference overall.
...
- The operator-directed reduced-distortion signal on the clean h16 branch still matters, and the newest excess-only retry says the objective shape is directionally useful, but the looser `2.0` cap still leaves the main clip and episode `2` too late-heavy.
- Best next test: resume the clean h16 step-`1000` checkpoint to step `1200` with `motion_loss_alpha=0.75`, `motion_loss_max_weight=1.5`, and `motion_loss_excess_only` so only above-average motion regions receive extra loss weight under the tighter cap.
...
- Practical ranking takeaway:
  ...
  - treat the capped and excess-only h16 `motion_loss_alpha=0.75` runs as proof that shaping the motion bonus helps, but not yet as the best h16 trade,
...
- If optimization resumes later, the next grounded move is the tighter-cap excess-only h16 continuation from the same clean step-`1000` checkpoint; if that still fails, switch to a stronger action-conditioning path.
