...
* [x] `scripts/check/dataset.py` loads `lerobot/libero` successfully
...
* [x] `scripts/check/masking_leakage.py`:
...
* [x] Run `scripts/check/masking_leakage.py` using masks generated by `block_causal.py` and chunk ids generated by `schedule.py`.
...
* [x] `scripts/check/forward_real_batch.py` verifies real-batch forward wiring with latent-time split, chunk ids, and block-causal mask.
...
* [x] `scripts/train/world_model.py` (end-to-end training/overfit script)
...
* [x] Implement `scripts/train/world_model.py`:
