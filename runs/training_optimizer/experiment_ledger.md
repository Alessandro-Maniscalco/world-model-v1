...
- `optimizer_aloha_static_fork_pick_up_full_320x240_ctx17_h4_lora32_noaction_gradckpt_residual_lastctx_filllastctx_singlechunk_fresh400` step `400`
  - `arm_motion_verdict`: main clip and held-out episode `1` remain `misaligned`, but held-out episode `2` upgrades to `good`
  - `image_quality_verdict`: this is the first easy residual branch after the reset that produces a plausible four-frame generated horizon instead of a single collapsing frame. On the main clip plus held-out episodes `1` and `2`, motion starts at frame `17` and continues through frames `18-20`; the main clip stays plausible but keeps the fork too far right with a bright soft tool/contact patch by frame `20`, held-out episode `1` is the weakest visually with the same right-shifted undercommit plus visible gripper/fork ghosting through the last frame, and held-out episode `2` is the cleanest with the closest trajectory match even though it is still slightly soft and right-shifted
  - `continue_training`: no plain continuation from the step-`400` checkpoint; evaluate the saved step-`300` checkpoint next
  - Why: the comparison clips and arm crops show the structural change worked in one important sense: frames `14-16` no longer lead into a single-frame collapse, and the generated side now moves through all last-horizon frames `17-20` while plausibility stays `PASS/PASS/PASS`. But motion-first ranking still rejects step `400` as a promotion candidate because the main clip and held-out episode `1` visibly undercommit near contact and keep blur/ghosting at the tool tip, while the training trace peaks much earlier (`best_val_loss≈0.0544` at step `300`, `≈0.2366` at step `400`). That makes checkpoint selection the highest-value bounded follow-up before declaring the local family exhausted.

- `optimizer_aloha_static_fork_pick_up_full_320x240_ctx17_h4_lora32_noaction_gradckpt_residual_lastctx_singlechunk_reval400` step `300`
  - `arm_motion_verdict`: all three windows stay `misaligned`; frames `14-16` still hold static/copy-like and only frame `17` diverges
  - `image_quality_verdict`: the checkpoint rescue does not clean up the final generated frame. On the main clip, frame `17` is still a beige/green fork-contact smear; held-out episode `1` is more washed out with visible gripper/fork ghosting and fails plausibility on frame `17`; held-out episode `2` is the brightest low-detail arm/contact blob and also fails plausibility on frame `17`
  - `continue_training`: no more plain checkpoint rescue on this scout; switch to the structural `future_control_fill_mode=last_context_frame` rerun on the same easy geometry
  - Why: the comparison clips and arm crops answer the checkpoint-selection question clearly. The predicted side matches the reference through frames `14-16`, then frame `17` breaks instead of committing cleanly. The reports confirm that all three windows remain `misaligned` (`late_motion_ratio≈2.08/2.42/2.58`, `profile_correlation≈0.26/-0.13/0.43`), and plausibility is now worse than the step-`400` checkpoint (`PASS/FAIL/FAIL` instead of `PASS/PASS/PASS`). Because the residual target still pairs the future control stream with gray filler latents, the next bounded test should change that control prior directly instead of retrying another scalar or checkpoint variant in the same family.

- `optimizer_aloha_static_fork_pick_up_full_320x240_ctx17_h4_lora32_noaction_gradckpt_residual_lastctx_singlechunk_reval400` step `400`
  - `arm_motion_verdict`: all three windows stay `misaligned`; the branch still only changes the final generated frame
  - `image_quality_verdict`: plausibility now passes on the main clip plus held-out episodes `1` and `2`, but frame `17` is still a low-structure fork/contact blur or blob on all three windows instead of a clean commit; episode `1` looks least trajectory-matched and episode `2` still carries the brightest low-detail contact patch
  - `continue_training`: no fresh continuation; do one checkpoint-selection pass on the saved step `300` checkpoint first
  - Why: the comparison clips and arm crops still show frames `14-16` matched on both sides and only frame `17` diverging. That last frame does not become a readable fork pose: the main clip ends in a smeared tool/contact patch, episode `1` ends in an even less trajectory-matched blob/ghosted patch, and episode `2` keeps a bright low-detail contact region even though plausibility passes. The reports confirm that all three windows remain `misaligned` (`late_motion_ratio≈1.73/1.99/2.61`, `profile_correlation≈0.49/0.04/0.42`), but the training trace also shows the branch is not fairly represented by the final checkpoint alone: `val_loss` improved to `≈0.0483` at step `300` and then regressed to `≈0.2269` by step `400`, and `step_0000300.pt` is available. That makes checkpoint selection the highest-value bounded follow-up before closing residual-target reformulation.

- `optimizer_aloha_static_fork_pick_up_full_320x240_ctx21_h4_lora32_action_noinputln_mlp128resid_gradckpt_singlechunk_reval200` step `200`
  - `arm_motion_verdict`: main and episode `2` stay `misaligned`; episode `1` turns `overactive`, but all three windows still only change the final frame
  - `image_quality_verdict`: frames `18-20` in the last horizon stay visually identical on both sides, then frame `21` collapses; the main clip and episode `1` end in a fork-shaped brown/green smear with visible blur around the tool/contact region, while episode `2` is worse with a brighter washed-out blob and missed contact
  - `continue_training`: no direct continuation or checkpoint-selection follow-up; close plain absolute-target easy action scouting and pivot to the simpler observation-only residual scout
  - Why: the validated comparison clips answer the rung cleanly. On all three windows, `frame21_gap` is the first large divergence between reference and prediction (`≈21.54/20.12/24.90`), while the maximum reference-prediction gap before frame `21` stays low (`≈1.40/1.38/1.23`). The arm-crop strips show the same pattern: no meaningful generated commitment through frames `18-20`, then a final-frame smear on the main clip, similar late blur/ghosting on episode `1`, and the worst collapse on episode `2`. The reports agree that this is not a held-out-safe base rung (`late_motion_ratio≈1.72/1.48/3.04`, `profile_correlation≈0.36/0.71/0.35`, plausibility `PASS/PASS/FAIL` with episode `2` failing on frame `21`). Training itself fit steadily to `best_val_loss≈0.1032` at step `200`, so another plain continuation is not the best use of the next long run.
...
- `optimizer_aloha_static_fork_pick_up_full_320x240_ctx21_h8_lora8_action_noinputln_mlp128resid_addedkv_gradckpt_fresh400` final checkpoint
  ...
  - Why: gradient checkpointing finally made the added-K/V backbone branch runnable, but the visual result is not a win. Reviewing the last `8` frames first shows the same pattern on all three windows: the fork stays parked for most of the horizon, then lunges forward too late in the final frames instead of matching the reference's earlier commitment. Episode `1` repeats that late snap, and episode `2` is later still. The arm-motion reports confirm that all three windows remain `misaligned`: main `late_motion_ratio≈2.275`, `profile_correlation≈0.277`, `mean_frame_mae≈3.03`; episode `1≈2.031`, `≈0.522`, `≈4.49`; episode `2≈3.173`, `≈0.467`, `≈3.15`, all with plausibility `PASS`. Training, however, improved steadily through step `350` (`best_val_loss≈0.0659`) before regressing at the final step (`val_loss≈0.0981` at step `400`), and the best saved checkpoint is step `300` (`val_loss≈0.0846`). That made step-`300` checkpoint selection the last bounded rescue before this whole added-K/V family should pivot out.
...
- `optimizer_aloha_static_fork_pick_up_full_320x240_ctx21_h8_lora8_action_noinputln_mlp128resid_addedkv_gradckpt_fresh400` step `300`
  ...
  - Why: the step-`300` rescue does not recover the branch. Reviewing the last `8` frames first, then the arm-crop strip and reports, showed the same long static hold followed by late motion. On the main clip, the fork stays near its starting pose through almost the whole last-horizon sheet and only begins its inward swing near the end. On held-out episodes `1` and `2`, that same late snap comes with visible blur/ghosting around the tool tip and bowl-contact region, which lines up with the plausibility failures on episode `1` frame `21` and episode `2` frames `21-22`. The controller result confirms the safety failure: main plausibility `PASS`, episode `1` plausibility `FAIL`, episode `2` plausibility `FAIL`, with all three arm-motion reports still `misaligned`. That means the only runnable backbone-route test inside Wan still fails the held-out safety gate even before its late regression at step `400`, so the whole added-K/V family is now exhausted.
...
- `optimizer_aloha_static_fork_pick_up_full_320x240_ctx21_h8_lora32_action_noinputln_mlp128resid_actiontokenaux1p0_resume800to1000`
  - `arm_motion_verdict`: no model result; training failed before the first checkpoint or any sweep artifacts were produced
  - `image_quality_verdict`: not applicable because there are no videos to review
  - `continue_training`: keep the same hypothesis and recover memory first; the best same-hypothesis retry is gradient checkpointing, not a new architecture pivot
  - Why: the run resumed from step `800`, correctly skipped optimizer restore because the checkpoint predates the new optional aux head, then OOMed on the first train step inside the Wan/VACE backbone while trying to allocate about `16 MiB`. No checkpoint, `metrics.jsonl`, or eval artifacts exist, so there is no model outcome to rank. Because the hypothesis is still completely untested on video quality and the failure size is small, the next highest-value action is to rerun the same action-token-aux resume with `gradient_checkpointing=true`.

- `optimizer_aloha_static_fork_pick_up_full_320x240_ctx21_h8_lora32_action_noinputln_mlp128resid_actiontokenaux1p0_gradckpt_resume800to1000` final checkpoint
  - `arm_motion_verdict`: plausible but still late-heavy and `misaligned` on the main clip plus held-out episodes `1` and `2`
  - `image_quality_verdict`: all three windows stay plausible and reasonably coherent, but the last-horizon sheets and arm crops still show the same long static hold followed by a late fork snap
  - `continue_training`: stop local action-conditioning retries; the whole action-path neighborhood is now exhausted on the best held-out-safe anchor
  - Why: reviewing the last `8` frames first, then the arm-crop sheets, showed the same failure pattern on all three windows. On the main clip, the fork barely changes pose for most of the last horizon and only lunges inward in the final frames, with obvious blur/ghosting once it finally moves. Episode `1` repeats that late rush with a stronger overshoot, and episode `2` is later still, staying parked almost until the final two frames. The canonical reports make that concrete: main `late_motion_ratio≈2.52`, `profile_correlation≈0.21`, `mean_frame_mae≈3.15`; episode `1≈2.04`, `≈0.50`, `≈3.31`; episode `2≈2.95`, `≈0.52`, `≈2.33`, all with plausibility `PASS`. Validation improved through step `950` (`best_val_loss≈0.0554`) before regressing at step `1000` (`val_loss≈0.2248`), but the last saved checkpoint before regression is only step `900`, so there is no strong checkpoint-selection rescue to justify another turn. This closes direct action-token supervision alongside the already-exhausted Wan-side routing and latent-projector families.
